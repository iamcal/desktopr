...
var ui = {
	...
	init: function() {
		...
		//photos.fileProtocolHandler = Cc['@mozilla.org/network/protocol;1?name=file'].getService(Ci.nsIFileProtocolHandler);
		//NOTE: worker = same as thumbnail, indexer = seperate
		...
		file.initWithPath("C:\\Documents and Settings\\Ashot Petrosian\\My Documents\\My Pictures");

		var paths = [];
		var process_dir = function(files) {
			while (paths.length < 100 && files.hasMoreElements()) {
				var f = files.getNext();
				f.QueryInterface(Components.interfaces.nsIFile);
				if (f.isDirectory()) {
					process_dir(f.directoryEntries);
				} else {
					var p = f.QueryInterface(Ci.nsILocalFile).path;
		...
		process_dir(file.directoryEntries);
		photos.add(paths);
		*/
		...
		//var subFile;
		//subFile = directoryEntries.getNext();
		//while(subFile){
